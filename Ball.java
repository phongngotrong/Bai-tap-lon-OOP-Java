public class Ball {
    ...
    public double calculateNewVelocityAngle(Rect paddle) {
        double relativeIntersectY = (paddle.y + (paddle.height / 2.0))
                - (this.rect.y + (this.rect.height / 2.0));
    ...
    public void update(double dt) {
        if (vy >= 0.0) {
            if (this.rect.y + (vy * dt) + this.rect.height > Constants.SCREEN_HEIGHT
        ...
        if (vx < 0.0) {
            if (this.rect.x + (vx * dt) < leftPaddle.x + leftPaddle.width) {
                if (this.rect.y + (vy * dt) > leftPaddle.y
                        && this.rect.y + (vy * dt) + this.rect.height < leftPaddle.y
                                + leftPaddle.height) {
            ...
            if (this.rect.x + (vx * dt) + rect.width > rightPaddle.x) {
                if (this.rect.y + (vy * dt) > rightPaddle.y
                        && this.rect.y + (vy * dt) + this.rect.height < rightPaddle.y
                                + rightPaddle.height) {
